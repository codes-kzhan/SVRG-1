...
/*
svm_SVRG_logistic(w, wtilde, ntilde, Z, lambda, eta, iterNum);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    double *w, *wtilde, *G, *Z, lambda, eta, innerProdI, innerProdZ, tmpDelta, c = 1, tmpFactor, *cumSum;

    if (nrhs != 7)
        mexErrMsgTxt("Function needs 7 arguments");
    ...
    for (i = 0; i < maxIter; i++)
    {
        ...
        else
        {
            for(j = 0; j < nVars; j++)
            {
                innerProdI += w[j] * Z[j + nVars * idx];
                innerProdZ += wtilde[j] * Z[j + nVars * idx];
            }
        }
