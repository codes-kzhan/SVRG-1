...
/*
SIGM_sparse(w,Z,lambda,eta,d,g);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    double *w, *wtilde, *G, *Z, lambda, eta, *u, *z, tau1, tau2, innerProdI, innerProdZ, tmpDelta;
    ...
    if (nrhs != 11)
        mexErrMsgTxt("Function needs 11 arguments");
    ...
    u = mxGetPr(prhs[7]);
    z = mxGetPr(prhs[8]);
    tau1 = mxGetScalar(prhs[9]);
    tau2 = mxGetScalar(prhs[10]);
    ...
    for (i = 0; i < maxIter; i++)
    {
        ...
        for(j = jc[idx]; j < jc[idx+1]; j++)
        {
            innerProdI += w[ir[j]] * Z[j];
            innerProdZ += wtilde[ir[j]] * Z[j];
        }
        ...
        for(j = jc[idx]; j < jc[idx+1]; j++)
        {
            z[ir[j]] -= eta / cZ * tmpDelta * Z[j];
            u[ir[j]] -= eta * tau1 / cU * tmpDelta * Z[j];
        }
